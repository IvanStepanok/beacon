...
| Item | Notes |
|---|---|
| **AI / computer-vision damage suggestion** | **IMPLEMENTED, advisory-only, fully on-device.** MobileNetV3Small transfer-learned on two ground-level datasets: California wildfire structure damage (DINS, CC-BY-4.0) + PEER Hub ImageNet Task 7 post-earthquake damage levels (CC-BY-NC-SA-4.0); the earthquake set was added specifically to fix the previously-weak middle tier. Held-out (4,234 imgs): overall 90.2% (95% CI 89.3–91.1), macro-F1 0.797; per-tier minimal F1 0.915 / partial F1 0.531 / complete F1 0.946; and on real earthquake imagery the partial tier reaches F1 0.662 (recall 0.761), up from F1 0.226 / recall 0.23 on the old wildfire-only model. *(The overall number is lower than the old 94.5% because the new test set is harder + more representative: 3× more 'partial' + real earthquake photos; both macro-F1 and the partial tier improved. See `ml/out/metadata.json`.)* Bundled as TFLite (Android) + Core ML (iOS), wired into capture as a suggestion the reporter confirms or overrides. Model-weights licence: CC-BY-NC-SA-4.0 (trained partly on PHI-Net → non-commercial / humanitarian use; Beacon *code* stays Apache-2.0). Domain: wildfire + ground-level earthquake now in-domain; flood/conflict still unvalidated (domain shift); advisory + human-in-the-loop framing is deliberate. |
...
- ✅ Connectivity self-heals (recompute on every callback + 4 s safety re-check), so the offline banner can't get stuck.
