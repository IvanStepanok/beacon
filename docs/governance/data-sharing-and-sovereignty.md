...
| **HIGH** | Report-level records (point lat/long, damage grade, timestamp, notes, photo — note: on-device face blur is **[Implemented]** but best-effort, so a stored photo may still contain a face the detector missed; licence-plate redaction is best-effort) | Internal, least-privilege RBAC only; export only under DSA with strong justification | DSA + DPO sign-off |
...
- **What is collected:** a photo (with metadata/EXIF removed *on your device* before anything is sent **[Implemented]**, and on-device face blurring applied before upload **[Implemented]**), your location, a damage rating, an anonymous ID, and any note you choose to add. *(On-device face blur is best-effort, not perfect, and licence-plate redaction is best-effort; the notice must therefore not promise that every face is blurred, and should still advise users to photograph structures, not people.)*
