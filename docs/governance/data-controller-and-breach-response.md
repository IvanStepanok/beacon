...
3. **Data minimization and purpose limitation** (Principles 2 and 3): Beacon collects only what is necessary for damage assessment; on-device anonymization (EXIF stripping **[Implemented]**; face/plate blurring **[Planned]**) is the first minimization control.
...
5. **Security by design and Accountability** (Principles 7 and 10): a **named controller**, least-privilege RBAC, encryption, an immutable audit trail, documented retention, and a tested incident SOP are mandatory, not optional.
...
| **Field validator** | Vetted field responder (mobile app) | Reports **assigned to them** for verification, within their assigned P-code area | Confirm/correct damage grade; mark verified/rejected; add validation note | Point-level **only for assigned tasks** in own area | See reports outside assigned area; bulk-export; edit RBAC; see submitter identity beyond pseudonym |
| **CO analyst / dispatcher** | Country Office staff (dashboard) | All reports **within their CO's country/admin areas**; point geometry for operational triage and tasking | Triage, assign tasks to validators, annotate, run **internal** exports (audited) | Point-level, **own country only** | Access other countries' data; approve external/HDX release; change global policy or keys |
...
- **The external boundary is hard.** Everything an external consumer sees has passed Statistical Disclosure Control and been coarsened to a P-code level signed off by the Crisis Bureau. Re-identification "by combining answers to different questions, even after anonymisation is applied" is the explicit risk SDC mitigates ([OCHA SDC Guidance Note](https://centre.humdata.org/guidance-note-statistical-disclosure-control/)).
...
| **On-device anonymization** | EXIF metadata is stripped **[Implemented]** before any image leaves the device. Face/licence-plate blur **[Planned] — NOT implemented** (only a boolean flag is stored; no detection/blur runs), so a face captured in shot is currently transmitted and stored unblurred. | **[Partial]** — EXIF strip Implemented; blur Planned |
...
- Server-side validation re-checks that uploaded images are anonymized; rejects non-conforming payloads — **[Planned]** (depends on the on-device blur pipeline, which is not yet built).
...
- **Secure destruction** uses cryptographic erasure (key destruction) plus deletion, and explicitly covers **offline device caches**, backups, and any processor copies (flowed down by the processing agreement).
...
| **1. Contain** | Treatment | Stop the bleeding before assessing fully: revoke/suspend the implicated account, rotate keys, take the affected export/endpoint offline, pull a wrongly-published HDX product, remote-wipe a lost device's queue if possible. Preserve the audit trail and evidence. | **Security Lead** + Data Steward | **Initial containment within 4 hours** of report |
