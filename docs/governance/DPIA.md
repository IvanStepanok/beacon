...
The **primary protection concern is the aggregate, not the individual record**. Individual records are anonymised at source to the extent currently built: EXIF stripping (**[Implemented]**) and pseudonymous submitter IDs (**[Implemented]**); on-device **face blur is [Implemented]** (ML Kit / Apple Vision, before upload), licence-plate redaction is best-effort, and a blur-failure fallback is **[Planned]** (see §6, R2). However, a dense, fine-grained map of where buildings are damaged is, in a crisis, a map of where vulnerable people are or recently were. Under OCHA and IASC frameworks this aggregate is **non-personal data that is nonetheless sensitive**, because the likelihood and severity of harm from its exposure are high in conflict and disaster settings (see [Centre for Humanitarian Data glossary](https://centre.humdata.org/glossary/)).

This DPIA concludes that Beacon can be deployed **subject to mandatory conditions** (§10). The dominant residual risks (re-identification from location clusters, misuse of photos of identifiable persons, and compulsion of data by a host government) are reducible but not eliminable. They are managed primarily through (a) never publishing raw coordinates (public-view coordinate coarsening (~110 m) + verified-only public reads are **[Implemented]** on the backend), (b) on-device anonymisation (EXIF stripping **[Implemented]**; on-device **face blur [Implemented]**, licence-plate redaction best-effort), (c) least-privilege RBAC and audit logging (**[Implemented]**), (d) a named controller and incident SOP, and (e) strict purpose limitation. The legal basis is **public interest / vital interest under UNDP's mandate, not consent**, because valid consent is generally unobtainable in a crisis (§2).
...
### 2.1 Purpose (purpose specification and limitation)
...
|---|---|---|---|---|
| Building photo (post-anonymisation) | Community member or field responder, on device | Possibly (if persons visible) | Medium–High | EXIF stripped on-device **[Implemented]**. On-device **face blur [Implemented]** (ML Kit / Apple Vision, runs before upload); licence-plate redaction is best-effort (text+aspect heuristic, not a trained detector). A blur-failure fallback (block/queue on low confidence) is **[Planned]** (§7, R2). |
...
[2] ON-DEVICE ANONYMISATION (privacy by design / by default)
    - EXIF metadata stripped (incl. embedded GPS, device serials, timestamps)
      [IMPLEMENTED]
    - Faces auto-detected and blurred on-device BEFORE the photo leaves the
      device  [IMPLEMENTED] (ML Kit / Apple Vision; licence-plate redaction is
      best-effort heuristic; a low-confidence blur-failure fallback is [PLANNED])
...
- **Least-privilege RBAC:** **[Implemented]** role scoping (5 roles incl. Country/Regional/Crisis) enforced server-side rather than only in the UI, with audit-actor derived from the token. Note: **public-view coarsening gating precise coordinates away from public reads is [Implemented]** (see R1).
...
| **R2** | **Photos of identifiable persons** (bystanders, residents) captured and stored; faces visible in interior shots. | Medium | High | **High** | **[Implemented]:** EXIF strip before storage; **on-device face blur (ML Kit / Apple Vision) runs before upload**; UI guidance to photograph structures not people; notes/photos excluded from default exports. **[Planned — NOT yet implemented]:** a blur-failure fallback (block/queue when detector confidence is low) and periodic QA sampling of stored photos; licence-plate redaction is **best-effort** (text+aspect heuristic), not a trained detector. **Residual risk remains where the face detector misses (no low-confidence fallback yet) or for licence plates.** |
...
| **C4** | **On-device face/plate blur + blur-failure fallback** (block/queue-for-review when detector confidence is low) and QA sampling in place (R2). *Status: on-device **face blur [Implemented]** (ML Kit / Apple Vision, before upload); licence-plate redaction is best-effort; the **blur-failure fallback + QA sampling remain [Planned]**.* |
